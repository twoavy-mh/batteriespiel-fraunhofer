...
public class FloorTilePlacer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            if (i % 10 != 0)
            {
                ...
                {
                    GameObject obstacle =
                        Instantiate(ObstacleTile, new Vector3(i * 3.42f, -3.5f, 0), Quaternion.identity);
    ...
    private float CalculateOffset()
    {
        ...
        Vector3[] bounds = Helpers.Utility.SpriteLocalToWorld(tile.transform,
            tile.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite);
